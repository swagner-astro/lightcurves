...
def fix_data(time, flux, flux_error):
    ...
    flux_error_ = flux_error[
        np.invert(np.isnan(flux)) * np.invert(np.isnan(flux_error))
    ]
...
def get_gti_iis(time, n_gaps, n_pick):
    ...
    diff = np.array([t - s for s, t in zip(time, time[1:], strict=False)])
    diff1 = np.sort(diff)
    ii = [
        x for x in range(len(diff)) if diff[x] in diff1[-n_gaps:]
    ]  # index of the 10 longest gaps
...
class LightCurve:
    ...
    def __repr__(self):
        ...
        return (
            f"LightCurve (bins = {len(self.flux)}, name = {self.name}, "
    ...
    def get_bblocks(self, gamma_value=None, p0_value=0.05):
        ...
        for j in range(len(self.edge_index) - 1):
            self.block_val[j] = np.mean(
                self.flux[self.edge_index[j] : self.edge_index[j + 1]]
            )
            self.block_val_error[j] = np.sqrt(
                np.sum(
                    self.flux_error[self.edge_index[j] : self.edge_index[j + 1]] ** 2
                )
    ...
    def plot_hop(self, ax=None, **kwargs):
        ...
        for i, hop in enumerate(self.hops):
            ...
            if i == 1:
                ...
                ax.fill_between(
                    x, y, y1, step="mid", color="lightsalmon", alpha=0.2, zorder=0
                )
